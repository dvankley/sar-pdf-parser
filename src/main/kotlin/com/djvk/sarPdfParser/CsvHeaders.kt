...
object CsvHeaders {
    ...
    enum class Fields(val docType: DocType, val csvFieldName: String, val pdfTableFieldNames: Set<String>? = null) {
        ...
        STUDENT_FIRST_NAME(DocType.SAR, "Student First Name", setOf("Student’s  First  Name")),
        STUDENT_MIDDLE_NAME(DocType.SAR, "Student Middle Name", setOf("Student’s  Middle  Initial")),
        STUDENT_LAST_NAME(DocType.SAR, "Student Last Name", setOf("Student’s  Last  Name")),
        STUDENT_DOB(DocType.SAR, "Student Date of Birth", setOf("Student's  Date  of  Birth")),
        STUDENT_SSN_L4(
            DocType.SAR,
            "Social Security Number Last 4 Digits",
            setOf("Student's  Social  Security  Number")
        ),
        PARENT_1_ED_LEVEL(DocType.SAR, "Parent 1 Educational Level", setOf("Parent  1  Educational  Level")),
        PARENT_2_ED_LEVEL(DocType.SAR, "Parent 2 Educational Level", setOf("Parent  2  Educational  Level")),
        ...
        STUDENT_GROSS_INCOME(
            ...
            setOf("Student's  2015  Adjusted Gross  Income")
        ),
        PARENT_GROSS_INCOME(
            DocType.SAR,
            "Parents'  2015  Adjusted Gross  Income",
            setOf("Parents'  2015  Adjusted Gross  Income")
        ),
        CHILD_SUPPORT_PAID(DocType.SAR, "Child Support Paid", setOf("Student's  Child  Support Paid")),
        STUDENT_HAS_CHILDREN(
            DocType.SAR,
            "Does student have children they support?",
            setOf(
                "Does Student Have  Children He/She Supports",
                "Does the Student Support Children",
            )
        ),
        STUDENT_HAS_DEPENDENTS(
            ...
            setOf(
                ...
                "Does the student support other dependents",
            )
        ),
        DECEASED_PARENTS_COURT_WARD_FOSTER_CARE(
            ...
            setOf(
                "Parents  Deceased?/Student  Ward  of  Court?/In  Foster Care",
                "Is the student a ward of court or in foster care or are the student's parents deceased",
            )
        ),
        EMANCIPATED_MINOR(
        ...
        IN_LEGAL_GUARDIANSHIP(
            ...
            setOf(
                ...
                "Is the student in a legal guardianship",
            )
        ),
        UNACCOMPANIED_HOMELESS_YOUTH(
            ...
            setOf(
                ...
                "Is the student unaccompanied and homeless as determined by a high school homeless liaison",
                "Is the student unaccompanied and homeless as determined by the U.S. Department of Housing and Urban Development",
                "Is the student unaccompanied and homeless as determined by a director of a homeless youth center",
            )
        ),
        PARENTS_RECEIVED_SNAP(
            DocType.SAR, "Parents received SNAP", setOf(
                "Parents  Received  SNAP",
                "Did the parent receive SNAP",
            )
        ),
        PARENTS_RECEIVED_TANF(
            DocType.SAR, "Parents received TANF", setOf(
                "Parents  Received  TANF",
                "Did the parent receive TANF",
            )
        ),
        STUDENT_RECEIVED_SNAP(
            DocType.SAR, "Student received SNAP", setOf(
                "Student  Received  SNAP",
                "Did the student receive SNAP",
            )
        ),
        STUDENT_RECEIVED_TANF(
            DocType.SAR, "Student received TANF", setOf(
                "Student  Received  TANF",
                "Did the student receive TANF",
            )
        ),
